...
function getRandomImage(callBack) {
  const settings = {
    ...
    data: {
      query: `${imageQuery}`,
      client_id:
        "cdf2e003c5fd56587721432496e946f53dcf21422a7a52212408e703929d26e5"
    },
    headers: {
      "Accept-Version": "v1"
    },
...
function processImageResponse(response) {
  ...
  $("html").css({
    background:
      "url(" + response.urls.regular + ") no-repeat center center fixed",
...
function processQuote(response) {
  ...
  const quoteText = `"${response.quote}"<br>- ${author}`;
...
function processMixCloudData(response) {
  ...
  function filterByTime(item) {
    if (userTimeLimit === "any length") {
      ...
      item.audio_length < userTimeLimit &&
      item.audio_length > userTimeLimit * 0.5
    ) {
      return true;
    } else {
      return false;
...
function renderNoResults() {
  const noResults =
    "sorry, we couldn't find any mixes based on your selection, try adjusting your search.";
...
function handleQuoteControls() {
  ...
  // handle clear button
  $("#clear-quote-btn").on("click", function() {
    $("#inspiration-container").fadeOut("slow", function() {
      $("#quote-text").empty();
    });
    $("#clear-quote-btn").fadeOut("slow");
  });
}
...
function renderTagOptions(selectedGenre) {
  ...
  tagSelector.html(
    '<option value="null" disabled selected>select tag</option>'
  );
...
function generateForm() {
  ...
  timeSelector.html(
    '<option value="any length" selected>no time limit</option>'
  );
...
function handleSubmit() {
  $("form").on("submit", function(e) {
    ...
    if (userInput.selected_genre === "electronic") {
      queryString =
        `${userInput.selected_style}+` + `${userInput.selected_tag}`;
    } else {
      queryString =
        `${userInput.selected_genre}+` + `${userInput.selected_tag}`;
    }
